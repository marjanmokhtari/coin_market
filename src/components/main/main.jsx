        ...
        <tbody className=" divide-y divide-line">
          {coin.map((coin, i) => (
            <tr key={coin.id} className="hover:bg-hoverblue">
              <td className='text-[14px] font-medium text-center'>{i + 1}</td>
              <td className="px-3 py-4 whitespace-nowrap   flex gap-2 items-center">
                <img className='w-6' src={coin.image} alt="" />
                <span className='text-[14px] font-medium'>{coin.name}</span>
              </td>
              <td className="px-3 py-4 whitespace-nowrap text-sm ">
                ${coin.current_price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </td>
              <td
                className={`px-6 py-4 whitespace-nowrap text-sm font-semibold text-center  ${coin.price_change_percentage_24h < 0 ? 'text-red-600' : 'text-green-600'
                  }`}
              >
